...
export default function GestionSubastasPage() {
  ...
  // ✅ Función para manejar la eliminación de subastas
        ...
        {Array.isArray(auctions) && auctions.length > 0 ? (
          auctions.map((auction) => (
            <li key={auction.id} className="mb-4 border p-4 rounded-md shadow-md bg-white">
              {/* ✅ Validamos que auction.product exista antes de acceder a sus propiedades */}
              <h2 className="text-2xl font-bold">
                {auction.product ? auction.product.name : 'Producto desconocido'}
              </h2>
